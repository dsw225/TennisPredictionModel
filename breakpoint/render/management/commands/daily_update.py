...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        asyncio.run(self.main(match_type))

    def add_arguments(self, parser):
        parser.add_argument('match_type', type=str, help="Match type: 'm': 'atp_matches_', 'w': 'wta_matches_', 'mc': 'atp_matches_qual_chall_', 'wc': 'wta_matches_qual_chall_', 'mi': 'itf_men_matches_', 'wi': 'itf_women_matches_'")

    async def main(self, mw):
        global proxy_url
        global auth
        proxy_url, auth = await self.read_proxies("data/scrapers/proxy_addresses/smartproxy.csv")

        games_data = await self.get_year_to_date(mw)
        self.save_data_to_csv(mw, games_data)
        ...
        matches_df = games_data.applymap(clean_data)
    ...
    async def get_year_to_date(self, mw):
        ...
        most_recent_entry = await self.get_most_recent_entry()

        games_in_year = []
        
        if most_recent_entry:
            day_of_year = current_date.timetuple().tm_yday
            day_most_recent = most_recent_entry.timetuple().tm_yday

            try:
                for day in range(day_most_recent+1, day_of_year):
                    ...
                    global exit_program
                    if exit_program:  # Check if exit_program is set
    ...
    def save_data_to_csv(self, mw, data):
        ...
        filename = f'data/csvs/Generated/{prefix}{datetime.date.today().year}_Update.csv'
    ...
    async def get_stats(self, mw, date):
        ...
        async with aiohttp.ClientSession(trust_env=True, connector=connector) as session:
            ...
            for match in unsorted_matches:
                ...
                match_odds = await self.fetch(session, base_url+match_odds_url, proxy_url, auth)
                if not match_odds:
                    print(f"Failed to fetch event odds for match ID {match_id}.")
                    continue

                match_stats = await self.extract_match_stats(match_info, match_stats, match_odds, date)
    ...
    async def extract_match_stats(self, match_info, match_stats, match_odds, date):
        try:
        ...
        odds = self.get_player_odds(match_odds.get("choices", {})) if match_odds.get("isLive", '') != True else (-1, -1)
        ...
        stats = self.initialize_stats(match_info_data, tourney_level, date, best_of, round_, minutes, odds)
        hand_table = {"right-handed": 'R', "left-handed": 'L'}
        
        if match_info_data.get('winnerCode') == 1:
            self.set_stats(stats, "w", "home", match_info_data.get("homeScore", {}), match_stats_all[0].get("statisticsItems", {}))
            self.set_stats(stats, "l", "away", match_info_data.get("awayScore", {}), match_stats_all[0].get("statisticsItems", {}))
            self.set_player_stats(stats, "winner", match_info_data.get("homeTeam", {}), match_info_data, "homeTeamSeed", hand_table)
            self.set_player_stats(stats, "loser", match_info_data.get("awayTeam", {}), match_info_data, "awayTeamSeed", hand_table)
        else:
            self.set_stats(stats, "w", "away", match_info_data.get("awayScore", {}), match_stats_all[0].get("statisticsItems", {}))
            self.set_stats(stats, "l", "home", match_info_data.get("homeScore", {}), match_stats_all[0].get("statisticsItems", {}))
            self.set_player_stats(stats, "winner", match_info_data.get("awayTeam", {}), match_info_data, "awayTeamSeed", hand_table)
            self.set_player_stats(stats, "loser", match_info_data.get("homeTeam", {}), match_info_data, "homeTeamSeed", hand_table)

        return stats

    def get_player_odds(self, match_odds):
        winner_odds = -1 
        loser_odds = -1
    ...
    def initialize_stats(self, match, tourney_level, date, best_of, round_, minutes, odds):
        return {
    ...
    def set_player_stats(self, stats, prefix, team, match, key, hand_table):
        stats[f"{prefix}_rank"] = team.get("ranking", '')
        stats[f"{prefix}_name"] = self.get_full_name(team.get("name", "N/A"), team.get("slug", "N/A"))
    ...
    def set_stats(self, stats, prefix, home_away, scores, stats_items):
    ...
    def get_full_name(self, name, slug):
